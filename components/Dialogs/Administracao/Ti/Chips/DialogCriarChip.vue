...
<!--              <span class="px-1 font-bold text-sm text-gray-600">Os dados do funcionário só será salvo após clicar em salvar</span>-->
              ...
              <div class="flex gap-3 items-end">
                ...
                <div class="flex gap-2 items-end grow">
                  ...
                  />
                  <div class="flex">
                    <BotaoPadrao cor="bg-blue-500 hover:bg-blue-600" @clique="buscarFuncionario()">
                      <img src="@/assets/icons/magnifier-w.svg" alt="" class="w-6 h-6">
                    </BotaoPadrao>
                  </div>
                </div>
              ...
              <div class="flex w-full gap-2 items-center">
                <div class="flex grow flex-col border border-slate-600 rounded" >
                  ...
                  <span v-if="chipLocal.data_entrega && Object.keys(funcionarioAtual).length > 0" class="px-2">Data de entrega: {{ $dayjs(chipLocal.data_entrega).format("DD/MM/YYYY") }}</span>
                  <span v-if="chipLocal.data_entrega && Object.keys(funcionarioAtual).length <= 0" class="px-2">Data de devolução: {{ $dayjs(chipLocal.data_entrega).format("DD/MM/YYYY") }}</span>
                </div>
                <img src="@/assets/icons/arrow-right-long-b.svg" alt="" class="w-8 h-9" v-if="buscouFuncionario">
                <div class="flex grow flex-col  border border-slate-600 rounded" v-if="buscouFuncionario">
                  <span class="bg-gray-300 px-2 rounded-t text-lg font-bold uppercase">Novo funcionário</span>
                  <div class="gap-4 flex px-2 justify-between">
                    <span :class="{'font-bold text-green-700': funcionarioLocal.nome === 'RETORNAR PARA O ESTOQUE'}">{{ funcionarioLocal.nome ? funcionarioLocal.nome : ""}}</span>
                    <span>{{ funcionarioLocal.chapa ? funcionarioLocal.chapa : ""}}</span>
                    <span>{{ funcionarioLocal.cargo ? funcionarioLocal.cargo : ""}}</span>
                  </div>
                  <span class="px-2">Data de entrega: {{ funcionarioLocal.data_entrega ? $dayjs(funcionarioLocal.data_entrega).format("DD/MM/YYYY") : $dayjs().format("DD/MM/YYYY")}}</span>
                </div>
              </div>
          ...
          <template v-slot:[`tab.historico`]="{ item }">
            <div class="max-h-[500px] overflow-y-auto">
              <div class="flex w-full justify-end px-2">
                <span>Total de historicos: <strong>{{ historicos.length }}</strong></span>
              </div>
              <div class="px-2 mt-1 flex flex-col gap-2">
              <div v-for="historico of historicos" :key="historico.id" class="flex flex-col p-1 bg-slate-300 border border-gray-400 rounded shadow">
                <div class="w-full flex justify-between">
                  <span><strong>Ação: </strong>{{ historico.acao }}</span>
                  <span><strong>{{ $dayjs(historico.createdAt).format("DD/MM/YYYY HH:mm:ss")}}</strong></span>
                </div>
                <span><strong>Funcionário: </strong>{{ historico.Funcionario && historico.Funcionario.nome ? historico.Funcionario.nome : "" }}</span>
                <span>
                  <strong>Usuário: </strong>{{ historico.Usuario.nome }}
                </span>
                  <span><strong>Observações</strong>
                  </span>
          ...
          <div v-if="tab === 'chip'" class="w-full flex gap-2 items-center">
            <div
          ...
          <div v-if="tab === 'funcionario'" class="w-full flex gap-2">
            <div
              class="text-red-500 text-xl mr-3"
              v-if="erros.length > 0">
              <span>Campos obrigatórios necessários</span>
...
<script>
    ...
    computed:{
      ...
      async buscarChip(){
        ...
        if(this.chipLocal.situacao_id === 2)
          camposObrigatorio.push('data_bloqueio')
        else if(this.chipLocal.situacao_id === 3)
          camposObrigatorio.push('data_cancelamento')
          ...
          if(!resp.falha){
			...
			async editarChip(sair) {
				...
				if (this.erros.length === 0) {
          let { id, numero, sim_card, data_ativacao, plano_id, situacao_id, tipo_servico, observacao, data_bloqueio, data_cancelamento } = this.chipLocal
          let chip = {
            id,
            numero,
            sim_card,
