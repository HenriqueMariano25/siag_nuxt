					...
					<div
						...
						<div
							...
							<span class="mt-2 uppercase">
								<strong>AÇÃO: </strong>{{ historico.AcaoHistoricoAtivoTI.nome }}
							</span>
							<span
								:class="{ 'text-green-900 font-bold': !historico.FuncionarioOrigem }"
								class="uppercase"
								v-if="historico.FuncionarioOrigem || historico.FuncionarioDestino">
								<strong class="!text-black">ORIGEM: </strong>
								{{ historico.FuncionarioOrigem ? historico.FuncionarioOrigem.nome : "Estoque" }}
							</span>
							<span
								class="uppercase"
								:class="{ 'text-green-900 font-bold': !historico.FuncionarioDestino }"
								v-if="historico.FuncionarioOrigem || historico.FuncionarioDestino">
								<strong class="!text-black">DESTINO: </strong>
								{{ historico.FuncionarioDestino ? historico.FuncionarioDestino.nome : "Estoque" }}
							</span>
							<span
								class="uppercase"
	...
	export default {
		...
		methods: {
			...
			async buscarHistorico() {
				...
				let resp = await this.$axios.$get("/ti/historico/ativo/buscar", { params: { id, modulo } })
				if (!resp.falha) {
