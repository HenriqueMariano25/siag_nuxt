  ...
  <div>
    <BaseDialog
      ...
      <template v-slot:corpo>
        <div class="px-2">
          <AppFormInput id="local_instalacao" type="text" label="Local de Instalação" v-model="nome" placeholder="Ex: CANTEIRO AVANÇADO" uppercase />
        </div>
      </template>
      <template v-slot:rodape-btn-direito>
        <div class="flex items-center gap-2">
          <span class="text-red-400 text-bold" v-if="jaCriado">Já existe esse local de instalação!</span>
          <BotaoPadrao texto="salvar" @clique="cadastrarLocalInstalacao()">
