<template>
  <div>
    <BaseDialog
      largura="w-8/12"
      :titulo="conhecimento ? 'EDITAR CONHE. FUNCIONÁRIO' : 'CADASTRAR CONHE. FUNCIONÁRIO'"
  ...
  methods: {
    ...
    async cadastrarConhecimento() {
      ...
      if (this.erros.length === 0) {
        let resp = await this.$axios.$post("/conhecimento_funcionario/cadastrar", {
          ...this.conhecimentoLocal
        })

        if (!resp.falha) {
          this.$emit("cadastrado", resp.dados.conhecimentoFuncionario)
        }
      }
    },
    ...
    async editarConhecimento() {
      ...
      if (this.erros.length === 0) {
        let resp = await this.$axios.$post(`/conhecimento_funcionario/editar`, {
          descricao, id
        })

        if (!resp.falha) {
          this.$emit("editado", resp.dados.conhecimentoFuncionario)
        }
      }
    },
    ...
    async deletarConhecimento() {
      ...
      let resp = await this.$axios.$delete("/conhecimento_funcionario/deletar", { params: { id } })
