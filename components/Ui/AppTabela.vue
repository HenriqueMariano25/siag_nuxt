<template>
  <div >
    <div class="grow" :style="'height:' + altura" style="overflow: auto; width: calc(100vw - 66px);"  >

      <table class="table-auto w-full">
        <colgroup>
          <col v-for="cab in cabecalho" :key="cab.valor" :class="{ [cab.tamanho]: cab.tamanho }">
        </colgroup>
        <thead >
        <tr class="">
          <th  v-for="cab in cabecalho" :key="cab.valor" class="uppercase px-2 py-1 text-sm text-white relative">
            <slot :name="'header.'+cab.valor" v-bind:item="cab">
              ...
              <div style="top: 34px;" v-if="filtroAberto === cab.valor"
                ...
                <div class="flex items-end">
                  <AppFormInput class="w-full" type="text" :label="'Buscar por '+ cab.nome"
                ...
                <div class="w-full mt-1">
                  <button @click.prevent.stop="removerFiltro(cab.valor)" class="bg-primaria-500 w-full py-1 hover:bg-primaria-700">
                    Limpar Filtro
                  </button>
                </div>
              </div>
        ...
        <tbody class="pt-10 overflow-auto !h-16" :style="'height:' + altura">
          <template v-for="dado in dados">
            <tr :class="{'!bg-gray-500 !text-white': trAberto === dado.id}"
                class="bg-white cursor-pointer even:bg-neutral-200 hover:bg-gray-600 hover:text-white" :key="dado.id"
                @click.prevent.stop="mostrarDetalhes(dado, $event)">
              <td v-for="(c, index) in cabecalho" :key="index" class="px-1 py-0.5 border border-collapse border-gray-600"
                  :class="{ 'text-center': c.centralizar }">
                <slot :name="'body.'+c.valor" v-bind:item="dado">
                  <span class="whitespace-nowrap">{{ dado[c.valor] }}</span>
                </slot>
              </td>
            </tr>

            <td :colspan="cabecalho.length" :id="`detalhes-${dado.id}`" v-if="trAberto === dado.id && temDetalhes"
                class="bg-blue-200 px-2 py-1 border border-gray-600">
              <slot name="detalhes" v-bind:item="dado"></slot>
            </td>
          </template>
          </tbody>
      </table>
    ...
    <div class="p-1 border h-16 border-gray-600 flex justify-between items-center bg-white">
      ...
      <div class="w-full flex justify-center space-x-1 items-center">
        <div>
          <button class="bg-slate-400 w-10 h-10 rounded hover:bg-slate-600 " @click.prevent.stop="anteriorPagina()">
            &lt;
          </button>
        </div>
        <div v-for="num of btnsPaginacao" :key="num">
          <button :class="{'!bg-blue-500': num === localPagina }"
                  class="bg-slate-400 w-10 h-10 rounded hover:bg-blue-600" @click.prevent.stop="escolherPagina(num)">
            {{ num }}
          </button>
        </div>
        <div>
          <button class="bg-slate-400 w-10 h-10 rounded hover:bg-blue-600" @click.prevent.stop="proximaPagina()">&gt;
          </button>
        </div>
      </div>
