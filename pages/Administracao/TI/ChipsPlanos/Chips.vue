<template>
	<div class="w-full print:!-ml-0.5">
		...
		<div class="w-full bg-white flex flex-col gap-2 overflow-y-auto !mt-11 print:hidden">
			<TabelaPadrao
				...
				<template v-slot:[`body.Funcionario.nome`]="{ item }">
					...
					<template v-if="!item.Funcionario">
						<span
							class="whitespace-nowrap bg-slate-600 !text-white p-1 rounded shadow"
							v-if="item.SituacaoChip && item.SituacaoChip.id === 3">
							CANCELADO
						</span>
						<span
							class="whitespace-nowrap bg-slate-600 !text-white p-1 rounded shadow"
							v-else-if="item.SituacaoChip && item.SituacaoChip.id === 4">
							TRANSFERIDO
						</span>
						<span
			...
			<template v-slot>
				<div class="flex justify-between w-full">
					<div class="flex gap-2">
						...
						<BotaoPadrao
							texto="KPI"
							@clique="mostrarDialogKpiChip = true">
							<img
		...
		<DialogKpiChip
			v-if="mostrarDialogKpiChip"
			@cancelar="mostrarDialogKpiChip = false" />
	</div>
</template>
		...
		methods: {
			...
			gerarExcel() {
				...
				for (let item of dados) {
					...
					temp.push(item.Funcionario ? item.Funcionario.nome : "No estoque")
					temp.push(item.Funcionario && item.Funcionario.setor ? item.Funcionario.setor.nome : "")
					temp.push(item.SituacaoChip ? item.SituacaoChip.descricao : "")
					itens.push(temp)
				}
