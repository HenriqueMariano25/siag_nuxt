	...
	<div class="w-full grow gap-y-2 flex flex-col">
      ...
      @dblclick="verAgendamentosFuncionario"
			...
			<template v-slot:[`body.selecione`]="{ item }">
				<div
					class="bg-red-50 rounded-full p-0.5 flex border-2 border-red-300 items-center justify-center"
					v-if="item.data_inicio_bloq_he !== null">
					<AppTooltip
						alinhamento="direita"
						:id="item.id">
						<template v-slot:corpo>
						...
						<template v-slot:tooltip>
							<div class="max-w-[420px] text-start text-md">
								<span> Funcionário bloqueado por faltas ou atestados na última competência. </span>
							</div>
						</template>
					</AppTooltip>
				</div>
				<div
					...
					v-else>
					<div
						class="bg-white rounded-full p-0.5 flex border-2 border-yellow-300 items-center justify-center"
			...
			<template v-slot:[`body.situacao`]="{ item }">
				<div
					v-if="item.data_inicio_bloq_he !== null"
					class="bg-red-500 !border-red-600 tag-situacao shadow">
					Bloqueado
				</div>
				<template v-else>
					<div
						v-if="temCamposVazio(item)"
						class="bg-sky-300 !border-sky-400 tag-situacao shadow">
						Sem dados
					</div>
					<div
						v-if="item.jaAgendado"
						class="bg-orange-400 !border-orange-500 tag-situacao shadow">
						Já agendado
					</div>
					<div
						v-if="item.aprovado"
						class="bg-green-400 !border-green-500 tag-situacao shadow">
						Aprovado
					</div>
					<div
						v-if="item.negado"
						class="bg-red-300 !border-red-400 tag-situacao shadow">
						Negado
					</div>
				</template>
			</template>
          ...
          v-if="agendamento.data < configuracaoHE.data_fim"
					...
					:class="{
						'font-bold  text-yellow-500':
							item.hora_extra_projetada >= 24 && item.hora_extra_projetada < 33.99,
						'font-bold  text-amber-700':
							item.hora_extra_projetada >= 34 && item.hora_extra_projetada < 39.99,
						'font-bold  text-red-600': item.hora_extra_projetada >= 40,
					}">
					{{
						item.hora_extra_projetada
		...
		methods: {
			...
			async buscarDiasPrAgendamento() {
				...
				if (!resp.falha) {
					...
					let diasAgendamento = await this.$axios
						.$get("/hora_extra/dias_agendamento/buscar_especifico")
						.then((resp) => resp.dados.dias)
