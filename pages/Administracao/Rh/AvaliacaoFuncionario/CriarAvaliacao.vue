<template>
  <div class="w-full">
    <CabecalhoPagina titulo="CRIAR AVALIAÇÃO " />
    <div class="w-full bg-white flex flex-col gap-2 overflow-y-auto !mt-11 print:hidden">
      <AppTabs
        :tabs="tabs"
        @tab="tab = $event">
        <template v-slot:[`tab.funcionario`]="{ item }">
          <div class="flex flex-col gap-2 overflow-y-auto" style="max-height: calc(100vh - 90px)">
            <div>
              <span class="text-gray-700 text-lg px-2 font-bold">Primeiro preencha os dados do funcionário </span>
            </div>
            <div class="grid grid-cols-12 p-1 gap-2">
      ...
      <template v-slot>
        <div class="flex items-center w-full justify-between">
          <div class="flex gap-2">
            <BotaoPadrao texto="voltar" @clique="voltar()">
              <img src="@/assets/icons/back-b.svg" alt="" class="w-7 h-7">
            </BotaoPadrao>
            <BotaoPadrao texto="cancelar" v-if="jaIniciado" @clique="cancelarAvaliacao" cor="bg-red-400 hover:!bg-red-500">
              <img src="@/assets/icons/delete-b.svg" alt="" class="w-7 h-7">
            </BotaoPadrao>
          </div>
          <div class="flex gap-2">
            <BotaoPadrao
              texto="SALVAR" @clique="iniciarAvaliacao(false)">
              <template v-slot>
                <img
            ...
            <BotaoPadrao
              texto="SALVAR e SAIR" @clique="jaIniciado ? editarAvaliacao(true) : iniciarAvaliacao(true)">
              <template v-slot>
                <img
                  src="@/assets/icons/save-b.svg"
                  alt="close"
...
export default {
  ...
  methods: {
    ...
    async iniciarAvaliacao(sair) {
      let { funcionario_id, formacao_id, anos_experiencia, conhecimentos, dispoTrabalho, tiposObra } = this.funcionario
    ...
    async buscarAvaliacao(id) {
      let resp = await this.$axios.$get("/avalicao_funcionario/buscarAvaliacao", { params: { id }})
