	...
	<div class="w-full">
		<div class="flex w-full bg-primaria-900 h-14 flex text-white">
			<button
				...
				v-if="$auth.user && $auth.user.permissoes.includes('aprovar_ss_controle')"
			...
			<button
				...
				class="px-4 hover:bg-blue-600"
				v-if="$auth.user && $auth.user.permissoes.includes('aprovar_ss_gerente_area')"
			...
			<button
				...
				class="px-4 hover:bg-blue-600"
				v-if="$auth.user && $auth.user.permissoes.includes('aprovar_ss_site_manager')"
		...
		<div class="">
			<AppTabela
				...
				<template v-slot:[`body.acoes`]="{ item }">
					<div class="w-8">
            <BotaoPadrao icone @clique="ss = item
              ...
              <img
				...
				<template v-slot:[`body.CentroCustoPEP.descricao`]="{ item }">
					<span
						v-if="item.CentroCustoPEP && item.CentroCustoPEP.descricao"
						class="whitespace-nowrap">
				...
				<template v-slot:[`body.EtapaSS.nome`]="{ item }">
					<span
						v-if="item.EtapaSS && item.EtapaSS.nome"
						class="whitespace-nowrap">
						{{ item.EtapaSS.nome }}
					</span>
				</template>
				...
				<template v-slot:[`body.comentarios`]="{ item }">
					<div class="border border-gray-400 bg-gray-300 !text-black w-full min-w-[185px] shadow">
						<button
							class="flex hover:bg-gray-400 w-full p-1 items-center"
